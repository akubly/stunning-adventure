...
prompt: |
  ...
  7. GIT COMMIT: Stage only the exact `.squad/` files Scribe wrote in this session. Use `git status --porcelain` filtered to allowed durable paths: decisions.md, decisions-archive.md, agents/{name}/history.md, agents/{name}/history-archive.md, identity/now.md. Stage each file individually with `git add -- <path>`. Handle renames by extracting destination path (`-replace '^.* -> ',''`). Commit with -F (write msg to temp file). Skip if nothing staged. ⚠️ NEVER use `git add .squad/` or broad globs. ⚠️ NEVER stage `.squad/log/*` or `.squad/orchestration-log/*` — these directories are gitignored runtime state; force-staging them bypasses .gitignore and is permanently prohibited.
