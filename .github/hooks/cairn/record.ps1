# Cairn Archivist — postToolUse hook wrapper
# Logs tool usage to knowledge.db
...
try {
    ...
    $p = Join-Path $HOME '.cairn\hook\postToolUse.mjs'
    ...
    if (-not $script) {
        ...
        if ($root) {
            $p = Join-Path $root '@akubly\cairn\dist\hooks\postToolUse.js'
            if (Test-Path $p) { $script = $p }
        }
    }
    ...
    if (-not $script) {
        $p = "$PSScriptRoot\..\..\..\dist\hooks\postToolUse.js"
        if (Test-Path $p) { $script = $p }
    }
