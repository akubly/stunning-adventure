...
📌 2026-06-09: **Issue #64 — Aperture icon correctness (B-1 + I-5)** — `getIcon` signature extended with optional `level?` parameter. Decision badge: `✓` → `📋`. Tier-aware fallback: urgent → `⚠️`, attention → `🔔`, other → `ℹ️`. 4 tests updated/added; 127/127 green. Build ✅ lint ✅. Issue closed. — Roger

## 2026-06-09: Issue #64 — Aperture Icon Correctness

### Changes
- `notification-policy.ts`: `getIcon(category, payload)` → `getIcon(category, payload, level?)`. Decision category returns `📋` (was `✓`). Tier-aware fallback: `urgent` → `⚠️`, `attention` → `🔔`, else → `ℹ️`.
- `aperture-projector.ts`: threads `level` into `getIcon` call.
- Tests: NP-4 updated (`✓`→`📋`), NP-5a+NP-5b added (⚠️/🔔), AP-2 now asserts `icon: '📋'`.

### Key lesson
`level?` is optional — the call signature `getIcon(category, payload, level?)` is backward-compatible (no existing callers break). Fallback chain: quarantine check → category check → level check → generic ℹ️.

 — Phase 2.5 inserted in `executeFlush()` (cas.syncAll before segment open). 6 new CAS-F tests, 3 group-commit tests updated. 125/125 tests GREEN. Build ✅, lint ✅. — Roger

## 2026-06-09: Issue #59 — CAS fsync Ordering

### Root cause
`FileSystemCas.put()` wrote CAS blobs via `fs.writeFileSync()` with no fsync. Phase 3 of `executeFlush()` called `syncFn(segFd)` on the segment, making WAL records durable while CAS blobs were still only in the OS page cache. A crash in that window left a durable WAL record referencing a non-durable CAS blob → `CasMissError` on reopen.

### Fix
Added `pendingSync: Set<string>` to `FileSystemCas`. `put()` adds the file path to `pendingSync` when a **new** file is written (not on dedup/already-exists cases). Added `syncAll(syncFn)` method: iterates `pendingSync`, opens each file with `'r+'` (write access required by `FlushFileBuffers` on Windows), calls `syncFn(fd)`, closes, removes from set. Files already synced in prior batches are not re-tracked.

Inserted **Phase 2.5** in `executeFlush()` between Phase 2 (hash chain) and Phase 3 (segment write): `this.cas.syncAll(this.syncFn)`. The abort path in Phase 2.5 rejects committed promises and re-queues restaged entries (segment not yet opened → no truncation needed).

**Ordering invariant (post-fix):** CAS blobs durable → segment written → segment durable.

### Throughput trade-off chosen
Batch CAS fsync within the group-commit barrier (Phase 2.5) rather than per-put fsync. Cost is O(K) new unique CAS files per flush, not O(rows). Dedup means repeated identical payloads never re-sync. Subsequent batches referencing the same CAS blob incur 0 CAS syncs. Amortised cost for workloads with payload repetition is far lower than O(rows) per-put syncing.

### Key file paths
- `packages/crucible-core/src/ledger/wal/cas-fs.ts` — `pendingSync` field + `syncAll()` method; updated class-level JSDoc
- `packages/crucible-core/src/ledger/wal-backend-fs.ts` — Phase 2.5 insertion in `executeFlush()` + updated JSDoc (step 4.5)
- `packages/crucible-core/src/__tests__/unit/wal-cas-fsync.test.ts` — 6 new tests (CAS-F1 through CAS-F6)
- `packages/crucible-core/src/__tests__/unit/wal-group-commit.test.ts` — updated 3 tests: syncCount assertions from 1→2, 1→2, 1→2/2→3 (second batch: same payload, 0 CAS syncs + 1 segment = 1 more sync)

### Interaction with #56
`#56` fixed manifest replay gate (unconditional `replayFromSegments()`). `#59` removes the vulnerability that `replayFromSegments()` would encounter: if CAS ordering is correct, no durable WAL record can exist without its CAS blob being durable first, so `CasMissError` can only occur from true hardware corruption, not from crash windows.

### Decision filed
`.squad/decisions/inbox/roger-cas-fsync.md`



## 2026-06-09: Issue #56 — WAL Crash-Durability Fix

### Root cause
`FileSystemWalBackend.open()` gated all replay on `manifest.lastCommitOffset >= 0`.
When the first batch crashed between Phase 3 (segment fdatasync) and Phase 4 (manifest.json writeFileSync), the manifest still showed `lastCommitOffset = -1`, so `replayFromSegments()` was never called. The segment bytes were durable but invisible on reopen.

For subsequent batches (manifest already `>= 0`), `replayFromSegments()` DID run and `scanSegmentFile()` read all bytes including post-crash rows — so the bug only manifested on the very first batch of a session.

### Fix (wal-backend-fs.ts, open())
Removed the `-1` gate. `replayFromSegments()` is now called unconditionally.
`scanSegmentFile()` returns `[]` for missing/empty segments, making the call a safe no-op for fresh sessions.

The manifest `lastCommitOffset` is still updated in Phase 4 for informational purposes (e.g., schema metadata). It is no longer used as a replay gate.

### Key file paths
- `packages/crucible-core/src/ledger/wal-backend-fs.ts` — removed `if (manifest.lastCommitOffset >= 0)` guard (~line 266), replaced with unconditional `replayFromSegments()` + explaining comment
- `packages/crucible-core/src/__tests__/unit/wal-crash-durability.test.ts` — 5 new tests (CD-1 through CD-5)

### Crash-injection test methodology
Crash simulation: write rows → flush (segment durable) → manually overwrite manifest.lastCommitOffset = -1 → close → reopen. This accurately models the on-disk state left by a crash between Phase 3 and Phase 4.

### Scope fence respected
WAL CAS fsync gap (issue #59) NOT touched. The fix does not change CAS write semantics.

### Decision filed
`.squad/decisions/inbox/roger-wal-crash-durability.md`

 — Implemented full RED→GREEN→REFACTOR cycle. 28 new tests, all green. Total suite: 114/114 (crucible-core) + 9/9 (crucible-cli). Build ✅, lint ✅. — Roger

## 2026-06-09: Crucible Walkthrough C — Aperture Push Notification

### Real ledger subscription seam (deviation from strategy doc)

The `Ledger` interface had **no `subscribe()` method**. The strategy doc's `ledger.subscribe(projector)` was illustrative. Added a minimal `LedgerSubscriber` interface + `subscribe(subscriber): void` to `Ledger` and `LedgerImpl`. Subscribers receive `onCommit(offset, event: LedgerEvent)` called synchronously after each successful `walBackend.commitRow()` resolves, before `append()` returns to the caller. Single-event callback (not batch) matches the current per-row commit model.

**Seam location:** `packages/crucible-core/src/ledger/ledger.ts` (types) + `packages/crucible-core/src/ledger/ledger-impl.ts` (implementation: step (e) after step (d) in `append()`).

### `metadata` field on `PrimitiveInput` (deviation from strategy doc)

`PrimitiveInput` had no `metadata` field. Added optional `metadata?: EventMetadata` (new interface with `level?: string` + `[key: string]: unknown`). Flows through to `LedgerEvent` automatically (via `Primitive extends PrimitiveInput`). Existing tests unaffected (field is optional).

### Projection DB seam (deviation from strategy doc)

The strategy doc showed `this.db.prepare('INSERT INTO aperture_events ...')` (SQLite DDL). The real backing store for projector tests is the in-memory WAL (no separate SQLite projection needed for the test slice). ApertureProjector uses an **internal `ApertureEvent[]` array** instead. `queryEvents()` returns a filtered snapshot. Same observable interface; avoids DDL coupling for this slice. A future adapter can replace the array with a projected SQLite table without changing the public API.
