...
📌 2026-06-27: **Crucible S4 WAL Substrate deepening — GAP-1, GAP-2, Session-reopen resolved (§3 Phase 1)** — Commit `25e976f` on `squad/s4-wal`. 264/264 tests ✅ (247 pre-existing + 17 new). tsc --noEmit ✅. Three deferred gaps from Phase 0.5 walking skeleton resolved.

**GAP-1 (flags.bootstrap, §3.8):** Added `WalRowFlags` interface and `walFlags?: WalRowFlags` to `PrimitiveInput` in `types.ts`. Defined separately from `SegmentRecordFlags` (in `wal/types.ts`) to avoid a circular import — `wal/types.ts` already imports from `types.ts`. Both `InMemoryWalBackend.commitRow()` and `FileSystemWalBackend.executeFlush()` now merge `input.walFlags` into the segment record's `SegmentRecordFlags` struct instead of hardcoding all-false. `LedgerImpl.bootstrap()` stamps `walFlags.bootstrap=true` on every row before calling `commitRow()` — callers get the bit automatically.

**GAP-2 (flush() on WalBackend, §3.5 group-commit atomicity):** Added `flush(): Promise<void>` to the `WalBackend` interface. `InMemoryWalBackend.flush()` is a documented no-op (commits immediately). `FileSystemWalBackend.flush()` was already public — now satisfies the interface. `LedgerImpl.bootstrap()` refactored: stages ALL rows via `commitRow()` without awaiting individually, then calls `walBackend.flush()`. With `batchSize >= N` the entire batch commits in one `fdatasync` barrier (true §3.8 atomicity). With default `batchSize=1` rows auto-flush sequentially (functional, not grouped). `hasBootstrapped` is set only AFTER `flush()` + `Promise.all()` succeeds — failed flush leaves it false, permitting a retry. BI-7 still passes (flaky mock backend gets `flush(): async () => {}`).

**Session-reopen:** Added `reopen?: boolean` to `LedgerFactoryOptions`. `LedgerImpl` constructor initializes `hasBootstrapped = reopen === true`. When `reopen:true`, `bootstrap()` throws immediately (already-bootstrapped guard) and `append()` works normally at the next available offset. `FileSystemWalBackend.create()` already replays segments on `open()` via `replayFromSegments()` — the backend side needed zero changes. Callers: `createLedger({ walBackend: reopenedFsBackend, reopen: true })`.

**Key learnings:**
- Circular import guard: when adding flag types to root `types.ts`, define a parallel interface rather than importing from `wal/types.ts` (which imports back from `types.ts`).
- Atomic bootstrap via group-commit requires staging ALL rows before awaiting any — `rows.map(r => commitRow(r))` followed by `await flush()` + `await Promise.all(promises)`. The JS event loop means multiple synchronous `commitRow()` calls before microtasks run can all land in the same staging queue flush.
- Session-reopen is entirely a Ledger-layer concern — the FS backend already handles replay on `create()`; only `hasBootstrapped` initialization in `LedgerImpl` needed to change.
— Roger## Learnings (2026-06-16 — Crucible S3 Phase 0.5 Walking Skeleton T2: bootstrap + replay)
