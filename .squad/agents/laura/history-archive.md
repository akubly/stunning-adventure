...
**Collaborator seam driven:** `TrustUpdater { update(args: { factId, sessionId, trust }): Promise<void> }`

**Contract chosen:** `applyFeedback(options, deps)` — separate from `recall()`. Options carry `currentTrust` (caller-provided); `deps` carries `trustUpdater` + `clock` (required, per §55 §1.2). Delta computation (±0.10 corroboration/contradiction) is pure logic inside the activity; TrustUpdater owns the write side only.

**Test file:** `packages/eureka/src/activities/__tests__/recall-feedback.test.ts` — 4 tests, all RED for `applyFeedback is not a function`. M1–M4 (18 tests) all green.

**§30 §2.3 spec gap discovered:** The section "Trust Dynamics Beyond the Static Floor" is cited in decisions.md but does NOT exist as a heading in 30-learning-systems.md (numbering jumps 2.2 → 2.4). Delta values (+0.10, -0.10, ±0.30) taken as authoritative from decisions.md Named M5 Target.

**user_correction ambiguity deferred:** The ± sign for user_correction (+0.30 or -0.30) must come from somewhere. Chosen interpretation: caller provides a signed `correctionDelta`. Test deferred; Edgar resolves interface shape in GREEN.
