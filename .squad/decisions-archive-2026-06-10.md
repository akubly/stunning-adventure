...
# 2026-06-10: Telemetry → Execution Profile Pipeline — APPROVED (Forge next milestone)

**Date:** 2026-06-10
**Author:** Squad Coordinator (capturing Aaron's approval)
**Status:** APPROVED — build deferred to next session
**Source:** Graham scope proposal (telemetry-profile-pipeline-scope) + Aaron decisions

## Confirmed gap
No production path creates `execution_profiles` from telemetry. `upsertExecutionProfile()` (cairn/src/db/executionProfiles.ts:102) is test-only; the forge telemetry collectors + `aggregateSignals` (forge/src/telemetry/*) are never wired into production; `skill_id` is absent from the event log and unset by collectors. Net effect: on a fresh install no profiles form → the forge prescriber emits no hints → the dogfood loop never bootstraps. The M3 disposition-feedback half is wired; the upstream telemetry→profile→prescriber half is not. All ExecutionProfile metrics ARE derivable from existing collector output — the gap is wiring, not data.

## Approved design (Aaron, 2026-06-10)
1. Build profiles inside `Curator.curate()` (before `sweepChangeVectors`) via a mockable pure-function seam `buildProfiles(db, options): BuildResult`. (Option A + Option C seam.)
2. Inject `skill_id` at collector-creation time; NULL-tagged samples fold into a global profile.
3. v1 granularity: `per-skill/global` + `global/global` fallback. Defer per-model/per-user to v2.
4. Build a `forge-seed-profile` bootstrap CLI as an escape hatch for immediate dogfooding.
5. Locate the runtime composition root to unblock signal-writer wiring (Slice 4).
6. Parallelize independent RED Slices 1+2 only; serialize 3→4→5. Never break London RED→GREEN→REFACTOR.

## Slices
1 skill_id propagation [Roger] · 2 ProfileBuilder pure fn [Alexander] · 3 Curator integration [Roger] · 4 signal-writer production wiring [Roger+Laura, true unblock] · 5 e2e verification [Laura]. Plus the bootstrap CLI (independent).

## Follow-up
Dogfooding docs (held on branch squad/forge-dogfooding-docs, commit 3fac92d, unpushed) document the current bootstrap barrier; update + ship that docs PR once this pipeline lands.

## Build status
Not started — Aaron will kick off next session from the prepared kickoff prompt.

---

# Telemetry → Execution Profile Pipeline — Design Scope

**Date:** 2026-06-10  
**Author:** Graham (Lead / Architect)  
**Status:** Proposal (awaiting Aaron's review)  
...
There is **no production code path** that creates `execution_profiles` rows from recorded telemetry. The only writer, `upsertExecutionProfile()` (`packages/cairn/src/db/executionProfiles.ts:102`), is called exclusively from tests. Separately, the forge telemetry collectors (`createDriftCollector`, `createTokenCollector`, `createOutcomeCollector` in `packages/forge/src/telemetry/collectors.ts`) and the aggregator (`aggregateSignals` in `packages/forge/src/telemetry/aggregator.ts`) exist as library code but are **never wired into any production runtime** — they are only exercised in test files (`feedback-loop.test.ts`, `telemetry-aggregator.test.ts`, `telemetry-collectors.test.ts`).

**Confirmed evidence:**

- `upsertExecutionProfile` grep: every call site is in `__tests__/` — zero production callers.
- `createDriftCollector`/`createTokenCollector`/`createOutcomeCollector` grep against `packages/runtime-cli/src` and `packages/skillsmith-runtime/src`: zero matches.
- `aggregateSignals` grep: only called from test files and docs.
- The `signal_samples` table (migration 011, `packages/cairn/src/db/migrations/011-telemetry-feedback.ts:9`) has a `skill_id TEXT` column (nullable) — but the Cairn event log (`packages/cairn/src/db/events.ts:16`) has no `skill_id` column; skill identity lives only in the JSON payload if the caller includes it. Neither `recordToolUse` nor `recordError` in the archivist (`packages/cairn/src/agents/archivist.ts:80-115`) accept or emit a `skill_id`. The hooks (`sessionStart.ts`, `postToolUse.ts`) have zero references to `skill_id`.

**Consequence chain:**

1. No signal samples are written in production → the `signal_samples` table stays empty.
2. No aggregation runs → `execution_profiles` stays empty.
3. The forge prescriber (`packages/forge/src/prescribers/forgePrescriberOrchestrator.ts:13`) requires an `ExecutionProfile` → no profile → no hints generated.
4. No hints → `sweepChangeVectors` (`packages/cairn/src/agents/curator.ts:628`) finds no applied hints → `computedSkillIds` stays empty.
5. The dogfood loop never bootstraps.

The M3 disposition-feedback half (hint lifecycle + change vectors) is fully wired. The **upstream** "telemetry → execution profile → prescriber" half has no production trigger.

---

## 2. Core Design Decision: WHERE the Profile Builder Runs

### Definitions

- **"Profile builder"** = the component that (a) reads `signal_samples`, (b) calls `aggregateSignals`, and (c) calls `upsertExecutionProfile` to persist the result.
- **"Signal writer"** = the component that runs collectors on the bridge event stream and persists `SignalSample` rows to `signal_samples`. This is a prerequisite to the builder, scoped separately below (§3).

### Option A: Inside `Curator.curate()` — after event processing, before `sweepChangeVectors`

```
curate() → process events → [BUILD PROFILES HERE] → sweepChangeVectors → runPrescribers
```

**Pros:**
- Single transaction context — profile build + change-vector sweep + prescriber orchestration happen atomically.
- Curator already owns the cursor-based "process-then-advance" pattern; profile build is a natural extension.
- Profile is guaranteed fresh before `sweepChangeVectors` reads it (no stale-profile window).
- Testability is high: `curate()` is already the most-tested pipeline; adding a step is incremental.

**Cons:**
- Adds latency to the session-start hook (Curator already has a 3s time budget — `TIME_BUDGET_MS`, curator.ts:127). Profile aggregation is ~O(n) in signal sample count.
- Couples profile-build lifecycle to Curator's event cursor. If aggregation is expensive, it competes with event processing for the time budget.

### Option B: In `runSessionStart()` as a distinct step, after `curate()` but before `prescribe()`

```
runSessionStart() → catchUp → curate() → [BUILD PROFILES HERE] → incrementSessionCounter → prescribe()
```

**Pros:**
- Separates concerns: Curator stays focused on event → insight; profile build is its own step.
- Can have its own time budget independent of Curator's.
- Easier to skip/defer when latency is tight (e.g., fast-path sessions).

**Cons:**
- Profiles may be built from stale signal data if new events arrive between curate() and profile build (unlikely in practice — these run sequentially in the same hook invocation).
- Adds another top-level step to the hook, which is already approaching its latency budget.
- Must duplicate the "which skills to build for" discovery that Curator already partially does.

### Option C: Standalone `ProfileBuilder` invoked by both Curator and session-start hook

A dedicated `ProfileBuilder` class/module with its own interface (`buildProfiles(db, options): BuildResult`), called from Curator *and* available as a standalone CLI command.

**Pros:**
- Maximum reusability: Curator calls it, session-start calls it, CLI tools call it.
- Clean interface seam for London-school TDD — mock the builder in Curator tests.
- Can be run manually for debugging (`node dist/profileBuilder.js --skill X`).

**Cons:**
- More boilerplate (another module, another interface) for what is currently a single call site.
- Risk of inconsistent invocation (two callers with different timing could produce conflicting profiles). Mitigated by `upsertExecutionProfile`'s idempotent ON CONFLICT clause, but still a coordination concern.

### ✅ RECOMMENDATION: Option A (inside Curator.curate), with Option C's interface seam

**Reasoning:**
1. **Bootstrap correctness:** The profile MUST be fresh before `sweepChangeVectors` reads it. Option A guarantees this by construction. Options B and C require careful ordering discipline.
2. **Latency:** `aggregateSignals` is benchmarked at <50ms for 1000 samples (see `feedback-loop.test.ts:582`). The signal_samples table is TTL-capped at 10K rows and 7 days (`sweepSignalSamples`, `enforceSignalSampleCap` in `signalSamples.ts:152-176`). This fits comfortably within Curator's 3s budget.
3. **TDD interface:** Extract the build logic into a pure function (like Option C) that takes `(db, options) → BuildResult`, but *call* it only from Curator. This gives Curator tests a mockable seam without the coordination risks of multiple callers.
4. **Curator already orchestrates** the post-build steps (sweep + prescriber). Adding the pre-step keeps the pipeline linear and auditable.

---

## 3. Signal Writer — skill_id Derivation

### Current State

- The `signal_samples` table has a nullable `skill_id TEXT` column (migration 011).
- The `SignalSampleInsert` type (`packages/cairn/src/db/signalSamples.ts:9`) has `skillId?: string | null`.
- The forge collectors (`packages/forge/src/telemetry/collectors.ts`) produce `SignalSample` objects but **do NOT set `skillId`** — the field is absent from the flush output (see `flush()` implementations at lines 187-204, 254-275, 316-331).
- The `CairnBridgeEvent` type and `attachBridge` function (`packages/forge/src/bridge/index.ts:202`) carry a `sessionId` but no `skillId`.
- The Cairn event log (`event_log` table) has no `skill_id` column at all.

### Gap Assessment

**skill_id is not currently derivable from the event stream.** The bridge events carry session context (sessionId, eventType, payload) but no skill identity. The `SignalSample.skillId` field exists in the schema as a forward-looking placeholder but is never populated.

### Design: How to Tag Samples with skill_id

**The skill_id must be injected at collector-creation time**, not derived from events:

1. When the runtime (skillsmith-runtime or runtime-cli) creates collectors for a session, it already knows which skill is being exercised (the `--skill` argument to `forge-prescribe`, or the MCP tool's `skill_id` parameter).
2. Modify the collector factory functions to accept an optional `skillId: string` parameter. When present, the `flush()` output includes it on the `SignalSample`.
3. The `LocalDBOMSink.persistSample()` call writes it to `signal_samples.skill_id`.

**This is a prerequisite sub-task** — without it, the profile builder cannot group samples by skill.

**Alternative considered:** Derive `skill_id` from the session's `repo_key` + heuristic. Rejected: a session may exercise multiple skills (or none), and heuristic derivation is fragile.

### Fallback for un-tagged samples

Signal samples with `skill_id IS NULL` should be aggregated into a `global` granularity profile only. This allows the system to produce global-level profiles from day one, even before per-skill tagging is fully wired.

---

## 4. Metric Aggregation Mapping

| ExecutionProfile Field | Source Data | Aggregation | Source Status |
|---|---|---|---|
| `sessionCount` | Count of distinct `session_id` values in consumed `signal_samples` | Increment per new session | ✅ Available (session_id is on signal_samples) |
| `drift.mean` | `signal_samples` where `kind='drift'`, `value` column | Weighted running mean (`aggregator.ts:113`) | ✅ Available (drift collector flush sets `value` = drift score) |
| `drift.p50` | Same drift values | Streaming quantile sketch (`aggregator.ts:122`) | ✅ Available |
| `drift.p95` | Same drift values | Streaming quantile sketch (`aggregator.ts:126`) | ✅ Available |
| `drift.trend` | Direction of recent drift values | Last-vs-first comparison (`aggregator.ts:128`) | ✅ Available |
| `drift.sketch` | Bucket histogram over [0,1] | Incremental update (`aggregator.ts:24`) | ✅ Available |
| `tokens.meanInputTokens` | `kind='token'` samples, `metadata.totalInput` | Weighted running mean | ✅ Available (token collector tracks `totalInput`) |
| `tokens.meanOutputTokens` | `kind='token'` samples, `metadata.totalOutput` | Weighted running mean | ✅ Available |
| `tokens.meanCacheHitRate` | `kind='token'` samples, `metadata.cacheHitRate` | Weighted running mean | ✅ Available |
| `tokens.totalCostNanoAiu` | `kind='token'` samples, `metadata.costNanoAiu` | Cumulative sum | ✅ Available |
| `outcomes.successRate` | `kind='outcome'` samples, `metadata.succeeded` | Running ratio | ✅ Available |
| `outcomes.meanConvergenceTurns` | `kind='outcome'` samples, `metadata.turnCount` | Weighted running mean | ✅ Available |
| `outcomes.toolErrorRate` | `kind='outcome'` samples, `metadata.toolErrorRate` | Weighted running mean | ✅ Available |
| `signals` (ProfileSignals) | `kind='drift'` samples, `metadata.signals.*` | Per-signal weighted means (`aggregator.ts:186-240`) | ✅ Available (drift flush includes `signals` in metadata) |

**All metrics are fully derivable from the existing collector output.** The aggregator (`aggregateSignals`) already implements the complete mapping. The gap is purely in the **wiring**: collectors → sink → signal_samples → aggregator → execution_profiles.

---

## 5. Granularity Tiers

### How the prescriber's fallback selects profiles

`loadExecutionProfile` (`packages/skillsmith-runtime/src/runtime.ts:191`) implements a tiered fallback chain:

1. **Always tried first:** `per-skill` / granularity_key `'global'`
2. **If `fallbackPolicy === 'full-chain'`:**
   - `per-model` / granularity_key = `fallbackContext.modelId`
   - `per-user` / granularity_key = `fallbackContext.userId`
   - `global` / granularity_key `'global'`

### v1 Recommendation: Build `per-skill/global` only

**Reasoning:**
1. `per-skill/global` is the primary tier — every prescriber call tries it first.
2. Building `per-model` and `per-user` requires additional context (model identity, user identity) that the collectors don't currently capture and that the signal_samples schema supports but doesn't enforce.
3. The `global` fallback tier (across all skills) is cheap to add as a bonus — just aggregate all signal_samples regardless of skill_id.
4. **Unblocking the loop requires exactly one tier:** `per-skill/global`. The fallback chain handles the "no profile at this tier" case gracefully today.

**v1 tiers to build:**
- `per-skill` / `'global'` — one profile per skill_id
- `global` / `'global'` — one profile across all skills (fallback safety net)

**Deferred to v2:**
- `per-model` — requires model_id on signal_samples (schema column exists but is not populated)
- `per-user` — requires user_id context propagation

---

## 6. TDD-First Milestone Breakdown

### Seam Architecture

```
┌──────────────────────────────────────────────────────┐
│  Signal Writer (production wiring)                   │
│  ┌──────────┐   ┌──────────┐   ┌────────────────┐   │
│  │Collectors│──▶│   Sink   │──▶│ signal_samples │   │
│  │(+skillId)│   │(DBOM)    │   │    (SQLite)    │   │
│  └──────────┘   └──────────┘   └────────────────┘   │
└──────────────────────────────────────────────────────┘
                        │
                        ▼
┌──────────────────────────────────────────────────────┐
│  Profile Builder (called from Curator)               │
│  ┌────────────────┐   ┌───────────────┐   ┌───────┐ │
│  │ signal_samples │──▶│aggregateSignals│──▶│upsert │ │
│  │   (read)       │   │   (pure fn)   │   │Profile│ │
│  └────────────────┘   └───────────────┘   └───────┘ │
└──────────────────────────────────────────────────────┘
                        │
                        ▼
        sweepChangeVectors → prescribers (existing)
```

**Key seams for mocking:**
- `SignalSampleReader` — reads from `signal_samples` grouped by skill_id (mockable in builder tests)
- `ProfilePersister` — wraps `upsertExecutionProfile` (mockable in builder tests)
- `aggregateSignals` — already a pure function, directly testable

### Milestone Slices

#### Slice 1: skill_id Propagation (prerequisite)
**RED:** Collector factories accept `skillId`; flush output includes it on SignalSample.  
**GREEN:** Modify `createDriftCollector`, `createTokenCollector`, `createOutcomeCollector` to accept optional `skillId` and propagate to flush output.  
**Owner:** Roger (cairn/runtime expertise).  
**Size:** Small (~2 files changed in forge/telemetry).

#### Slice 2: ProfileBuilder Pure Function
**RED:** Interface contract test: given signal_samples rows + existing profile (or null), produce an updated ExecutionProfile via `aggregateSignals` + persistence call.  
**GREEN:** `buildProfiles(db, options): BuildResult` — reads signal_samples grouped by skill_id, calls `aggregateSignals` per group, upserts results.  
**Owner:** Alexander (cairn DB layer expertise).  
**Size:** Medium (~1 new module + interface contract tests).

#### Slice 3: Curator Integration
**RED:** Curator test: after event processing, curate() calls profileBuilder and profiles exist before sweepChangeVectors.  
**GREEN:** Wire `buildProfiles` call into `curate()` between event processing loop and `sweepChangeVectors`.  
**Owner:** Roger (Curator owner).  
**Size:** Small (~1 file changed in curator.ts, mock-based tests).

#### Slice 4: Signal Writer Production Wiring
**RED:** Integration test: bridge events flow through collectors → sink → signal_samples rows appear in DB.  
**GREEN:** Wire collectors + LocalDBOMSink in the production runtime composition root. This is the most environment-dependent slice — requires understanding how the runtime initializes sessions.  
**Owner:** Roger (runtime wiring) + Laura (integration test authoring).  
**Size:** Medium-Large (composition root changes, integration test with real DB).

#### Slice 5: End-to-End Verification
**RED:** Smoke test: from a fresh DB, simulate N sessions with bridge events → verify execution_profiles rows exist → verify prescriber can generate hints.  
**GREEN:** No new production code — this is a test-only slice that validates the full pipeline.  
**Owner:** Laura (test specialist).  
**Size:** Small-Medium (test file only).

**Slice dependency order:** 1 → 2 → 3 → 4 → 5 (mostly serial; 2 and 4 can partially overlap since 2 uses mocked readers).

---

## 7. Risks

### R1: Schema/Semantics Lock-in
The `execution_profiles` table schema is already deployed (migration 011). The `ExecutionProfile` type (`packages/types/src/index.ts:187`) is a cross-package contract. Adding fields (e.g., `signals`, `confidence`, `staleness`) has been done safely via optional properties, but **changing the meaning of existing fields** (e.g., redefining `drift_mean` from a running mean to an EMA) would require a migration. The builder should use the existing `aggregateSignals` semantics exactly.

### R2: Hook Latency
The session-start hook (`sessionStart.ts`) is latency-sensitive — it fires on every first tool call. Adding profile build to Curator's pipeline adds O(signal_samples_count) work. Mitigation: the TTL sweep + row cap (`sweepSignalSamples`, `enforceSignalSampleCap`) bound the input size. The aggregator benchmarks at <50ms for 1K samples. Monitor via the existing `TIME_BUDGET_MS` cap mechanism.

### R3: Signal Writer Is the True Blocker
The profile builder is useless without signal samples. Slice 4 (Signal Writer Production Wiring) is the true unblock point. If this slice is delayed (e.g., because the composition root is complex), consider a **manual seeding CLI** as a temporary bootstrap mechanism: `forge-seed-profile --skill X --session-count N` that creates a synthetic profile to unblock the prescriber pipeline.

### R4: Concurrent Aggregation
If two Curator runs overlap (e.g., worktree A and worktree B both trigger curate() simultaneously), they could race on profile upsert. Mitigation: `upsertExecutionProfile` uses `ON CONFLICT DO UPDATE` which is atomic per SQLite's single-writer guarantee. Profiles may temporarily reflect only one worktree's data, but the next sweep will re-aggregate. Acceptable for v1.

### R5: No Source Data Gap Identified
All `ExecutionProfile` metrics are fully derivable from existing collector output (see §4). No metric is blocked on missing source data. This is a positive finding — the gap is purely in wiring, not in data availability.

---

## 8. Open Questions for Aaron

1. **Manual bootstrap escape hatch:** Should we build a `forge-seed-profile` CLI command as a temporary bypass for dogfooding before full pipeline wiring lands? This would let us manually populate profiles to test the prescriber → hint → change-vector feedback loop independently. Or is the full pipeline wiring the only acceptable path?

2. **Signal writer composition root:** The collectors need to be wired into the production runtime's event bridge. Where does the runtime currently create the CopilotSession / EventSource? Is this in the VS Code extension, the CLI, or a shared composition root? This determines Slice 4's scope and difficulty.

3. **Global fallback profile:** Should the builder produce a single `global/global` profile (aggregating all skills) in v1? This is cheap to add and gives the prescriber fallback something to work with even for skills that have no per-skill data. Recommended yes.

4. **Slice ordering priority:** Given the dependency chain, should we parallelize Slices 1+2 (they're independent), or strictly serialize for review bandwidth?
