...
| T5  | A predicate runs too long and blocks the WAL commit                                     | v1 cooperative 80 µs per-row budget; post-hoc `predicate_timeout` telemetry and retry-budget quarantine (§4.3) | `PredicateRegistration.evaluate` is synchronous, so v1 measures elapsed time after return; it cannot preempt an infinite loop or CPU-bound scan already on the event loop. Over-budget completions emit `predicate_timeout`, decrement retry budget, quarantine the predicate in memory, then stage durable unregistration after the current append when exhausted. | Honest v1 containment is convention + telemetry + future-row quarantine, not a sandbox. A single bad synchronous predicate can still block the current commit; true hard preemption is deferred to v1.5+ worker/process isolation or an async cancellable predicate API. |
