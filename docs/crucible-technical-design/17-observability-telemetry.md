...
| Predicate budget overrun               | L1    | attention           | `observation` / `predicate_timeout`                             | `predicateId`, `commitOffset`, `elapsedMicros`, `retryBudgetRemaining`     | §4.3           |
| Row budget exhausted                   | L1    | attention           | `observation` / `row_budget_exhausted`                           | `commitOffset`, `budgetMicros` (budget value per §4.3), `elapsedMicros`   | §4.3           |
